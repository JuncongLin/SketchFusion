...
CPLYFileManager::CPLYFileManager(void)
{
	per_vertex_color=0;
	has_normals=0;
}
...
PlyFile* CPLYFileManager::InterpretHead(FILE* fp)
{
	int i,j;
	PlyFile *plyfile;
	int nwords;
...
char** CPLYFileManager::get_words(FILE* fp, int* nwords, char** orig_line)
{
...
int CPLYFileManager::equal_strings(char* s1, char* s2)
{
	int i;
...
void CPLYFileManager::add_element(PlyFile* plyfile, char** words, int nwords)
{
	PlyElement *elem;
...
void CPLYFileManager::add_property(PlyFile* plyfile, char** words, int nwords)
{
	int prop_type;
	int count_type;
	PlyProperty *prop;
...
void CPLYFileManager::add_comment(PlyFile* plyfile, char* line)
{
	int i;
...
void CPLYFileManager::append_comment_ply(PlyFile* ply, char* comment)
{
...
void CPLYFileManager::add_obj_info(PlyFile* plyfile, char* line)
{
	int i;
...
void CPLYFileManager::append_obj_info_ply(PlyFile* ply, char* obj_info)
{
...
int CPLYFileManager::get_prop_type(char* type_name)
{
	int i;
...
char* CPLYFileManager::setup_element_read_ply(PlyFile* ply, int index, int* elem_count)
{
	PlyElement *elem;
...
void CPLYFileManager::ExtractElements(PlyFile* in_ply)
{
...
void CPLYFileManager::Read(FILE* file)
{
	PlyFile *in_ply;
	in_ply=InterpretHead(file);
	ExtractElements(in_ply);


}

void CPLYFileManager::setup_property_ply(PlyFile* plyfile, PlyProperty* prop)
{
	PlyElement *elem;
...
PlyProperty* CPLYFileManager::find_property(PlyElement* elem, char* prop_name, int* index)
{
	int i;
...
PlyOtherProp* CPLYFileManager::get_other_properties_ply(PlyFile* plyfile, int offset)
{
	PlyOtherProp *other;
...
void CPLYFileManager::get_element_ply(PlyFile* plyfile, void* elem_ptr)
{
...
PlyOtherProp* CPLYFileManager::get_other_properties(PlyFile* plyfile, PlyElement* elem, int offset)
{
	int i;
	PlyOtherProp *other;
	PlyProperty *prop;
...
void CPLYFileManager::ascii_get_element(PlyFile* plyfile, char* elem_ptr)
{
	int i,j,k;
	PlyElement *elem;
	PlyProperty *prop;
...
void CPLYFileManager::binary_get_element(PlyFile* plyfile, char* elem_ptr)
{
	int i,j,k;
	PlyElement *elem;
	PlyProperty *prop;
...
void CPLYFileManager::setup_other_props(PlyFile* plyfile, PlyElement* elem)
{
	int i;
	PlyProperty *prop;
	int size = 0;
...
void CPLYFileManager::copy_property(PlyProperty* dest, PlyProperty* src)
{
...
void CPLYFileManager::get_ascii_item(char* word, int type, int* int_val, unsigned int* uint_val, double* double_val)
{
	switch (type) {
	case Int8:
	case Uint8:
...
void CPLYFileManager::get_binary_item(FILE* fp, int type, int* int_val, unsigned int* uint_val, double* double_val)
{
	char c[8];
	void *ptr;
...
void CPLYFileManager::store_item(char* item, int type, int int_val, unsigned int uint_val, double double_val)
{
...
PlyOtherElems* CPLYFileManager::get_other_element_ply(PlyFile* plyfile)
{
	int i;
	PlyElement *elem;
	char *elem_name;
...
void CPLYFileManager::close_ply(PlyFile* plyfile)
{
  fclose (plyfile->fp);
}

void CPLYFileManager::free_ply(PlyFile* plyfile)
{
  free (plyfile);
}

PlyOtherProp* CPLYFileManager::ply_get_other_properties(PlyFile* plyfile, char* elem_name, int offset)
{
	PlyElement *elem;
	PlyOtherProp *other;
...
void CPLYFileManager::ply_get_element(PlyFile* plyfile, void* elem_ptr)
{
	if (plyfile->file_type == PLY_ASCII)
		ascii_get_element (plyfile, (char *) elem_ptr);
	else
...
PlyElement* CPLYFileManager::find_element(PlyFile* plyfile, char* element)
{
	return NULL;
}

int CPLYFileManager::GetVertexNum(void)
{
	return nverts;
}

int CPLYFileManager::GetFaceNum(void)
{
	return nfaces;
}

void CPLYFileManager::GetVertexByIndex(int index, float& x, float& y, float& z)
{
	x=vlist[index]->x;
	y=vlist[index]->y;
	z=vlist[index]->z;
...
void CPLYFileManager::GetFaceByIndex(int index, int& v1, int& v2, int& v3)
{
	v1=flist[index]->verts[0];
	v2=flist[index]->verts[1];
	v3=flist[index]->verts[2];
}
