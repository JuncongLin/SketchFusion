...
public :
  ...
  inline MVertex   * vert( const int i ) const { if( i < 0  || i >= _nverts ) return ( MVertex *)NULL ; return _vertices  + i ; }
  ...
  inline MVertex   *vertices () { return _vertices  ; }
  ...
  inline void  set_cube_type  ( const int type, const int i, const int j, const int k ) { _cube_type[ i + j*_size_x + k*_size_x*_size_y] = type ; }
  ...
  void set_boundingbox(const CVector3d& vmin,const CVector3d &vmax);

  CVector3d get_grid_vert(const int i, const int j, const int k);
  void clarify_cubes(const std::vector<CPoint3>& bound1,const std::vector<CTriangle3>& triband);
  void determine_cubes_cases(real iso = (real)0.0);
...
protected :
  ...
  /** adds the boundary vertices */
  int add_b_vertices(const std::vector<CVector3d>& bound);
...
protected :
  ...
  MVertex   *_vertices   ;  /**< vertex   buffer */
  ...
  bool _bDraw;
public:
	CBox3& get_boundingbox(void);
	void GetCube(int i, int j, int k, CBox3& cb, int& ct);
};
//_____________________________________________________________________________
